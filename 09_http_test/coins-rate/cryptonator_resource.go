...
const (
	ErrTextCommonCriptonator = "[Cryptonator]"
	ErrTextNotOKCriptonator  = "[Cryptonator] not OK status code"
)
...
func (rcv *cryptonatorResource) BitCoinToUSDRate(ctx context.Context) (float64, error) {
	r, err := http.NewRequest("GET", fmt.Sprintf("%//api/ticker/BTC-USD", rcv.baseURL), nil)

	if err != nil {
		return 0, errors.Wrap(err, ErrTextCommonCriptonator)
	}
